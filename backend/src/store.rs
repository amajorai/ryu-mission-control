...
impl Store {
    ...
    pub fn index(&self, id: &str, req: &IndexRequest, now_ms: i64) -> Result<ConversationDigest> {
        ...
        tx.execute("DELETE FROM work_item WHERE conversation_id = ?1", params![id])?;
...
mod tests {
    ...
    fn days_bucket_by_local_date() {
        ...
        store.index("a", &req(1_700_000_000_000, &[], &[]), 1).unwrap();
        store.index("b", &req(1_700_000_000_000, &[], &[]), 1).unwrap();
        store.index("c", &req(1_800_000_000_000, &[], &[]), 1).unwrap();
