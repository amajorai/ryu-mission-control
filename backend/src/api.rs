...
mod openapi_tests {
    ...
    fn multi_method_paths_keep_every_operation() {
        ...
        for (path, methods) in [
            ("/api/mission-control/conversations/{id}", &["get", "put", "delete"][..]),
        ] {
            let item = wire
    ...
    fn the_index_body_is_typed_and_resolvable() {
        ...
        for name in ["IndexRequest", "Totals", "TurnRecord", "FileTouch", "WorkItem"] {
            assert!(
    ...
    fn the_window_queries_document_their_parameters() {
        ...
        for path in [
            ...
            let params = wire
                .pointer(&format!("/paths/{}/get/parameters", path.replace('/', "~1")))
